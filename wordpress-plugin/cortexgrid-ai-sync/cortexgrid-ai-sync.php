<?php
/**
 * Plugin Name: CortexGrid AI Sync
 * Description: Syncs WooCommerce products, blog posts and pages to the CortexGrid AI knowledge platform (manual "Sync now"). Re-syncs upsert by external_id, so data stays fresh without duplicates.
 * Version: 1.0.0
 * Author: CortexGrid AI
 * License: MIT
 */
...
const CORTEXGRID_AI_OPTION = 'cortexgrid_ai_settings';
const CORTEXGRID_AI_BATCH = 50;

/** Default settings. */
function cortexgrid_ai_settings(): array
{
    return wp_parse_args(get_option(CORTEXGRID_AI_OPTION, []), [
        'base_url' => '',
        'api_key' => '',
...
add_action('admin_menu', function () {
    add_menu_page('CortexGrid AI Sync', 'CortexGrid AI', 'manage_options', 'cortexgrid-ai', 'cortexgrid_ai_render_page', 'dashicons-rest-api', 80);
});

/** Fetch the tenant's widget-enabled agents (id, name, public_key, embed). */
function cortexgrid_ai_fetch_agents(array $s)
{
    ...
    if ($code < 200 || $code >= 300) {
        return new WP_Error('cortexgrid_http', 'HTTP '.$code);
    }
...
function cortexgrid_ai_render_page(): void
{
    ...
    // Save settings
    if (isset($_POST['cortexgrid_save']) && check_admin_referer('cortexgrid_ai_save')) {
        update_option(CORTEXGRID_AI_OPTION, [
    ...
    // Run a sync
    if (isset($_POST['cortexgrid_sync']) && check_admin_referer('cortexgrid_ai_sync')) {
        $type = sanitize_text_field($_POST['cortexgrid_sync']);
        $notice = cortexgrid_ai_run_sync($type);
    }

    $s = cortexgrid_ai_settings();
    ?>
    <div class="wrap">
        <h1>CortexGrid AI Sync</h1>
        <?php if ($notice): ?><div class="notice notice-info is-dismissible"><p><?php echo esc_html($notice); ?></p></div><?php endif; ?>

        <h2>პარამეტრები</h2>
        <form method="post">
            <?php wp_nonce_field('cortexgrid_ai_save'); ?>
            <table class="form-table">
                ...
                <tr><th>API Key</th><td><input type="text" name="api_key" value="<?php echo esc_attr($s['api_key']); ?>" class="regular-text" placeholder="cortexgrid_..."></td></tr>
            ...
            <?php
            $agents = (empty($s['base_url']) || empty($s['api_key'])) ? [] : cortexgrid_ai_fetch_agents($s);
            ...
            <p><button class="button button-primary" name="cortexgrid_save" value="1">შენახვა</button></p>
        </form>
        ...
        <form method="post">
            <?php wp_nonce_field('cortexgrid_ai_sync'); ?>
            <?php if (function_exists('wc_get_products')): ?>
                <button class="button" name="cortexgrid_sync" value="products">🛒 პროდუქტების სინქი</button>
            <?php else: ?>
                <button class="button" disabled title="WooCommerce არ არის აქტიური">🛒 პროდუქტები (WooCommerce საჭიროა)</button>
            <?php endif; ?>
            <button class="button" name="cortexgrid_sync" value="posts">📝 ბლოგის სინქი</button>
            <button class="button" name="cortexgrid_sync" value="pages">📄 გვერდების სინქი</button>
        </form>
    </div>
    <?php
}
...
function cortexgrid_ai_run_sync(string $type): string
{
    $s = cortexgrid_ai_settings();
    ...
    switch ($type) {
        case 'products': $records = cortexgrid_ai_collect_products(); $dataset = $s['ds_products']; break;
        case 'posts':    $records = cortexgrid_ai_collect_posts('post'); $dataset = $s['ds_posts']; break;
        case 'pages':    $records = cortexgrid_ai_collect_posts('page'); $dataset = $s['ds_pages']; break;
    ...
    $created = 0;
    $updated = 0;
    foreach (array_chunk($records, CORTEXGRID_AI_BATCH) as $batch) {
        $res = cortexgrid_ai_post_ingest($s, $dataset, $batch);
        if (is_wp_error($res)) {
...
function cortexgrid_ai_post_ingest(array $s, string $dataset, array $records)
{
    ...
    if ($code < 200 || $code >= 300) {
        return new WP_Error('cortexgrid_http', 'HTTP '.$code.': '.wp_remote_retrieve_body($response));
    }
...
function cortexgrid_ai_collect_products(): array
{
    $records = [];
...
function cortexgrid_ai_collect_posts(string $postType): array
{
    $records = [];
